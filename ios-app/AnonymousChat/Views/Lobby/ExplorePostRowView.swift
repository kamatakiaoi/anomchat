...
public struct ExplorePostRowView: View {
    ...
    private var mediaPreviewSection: some View {
        if !post.allImages.isEmpty {
            imagesGalleryView(images: post.allImages)
            ...
            audioPlayerRow(audUrl: audUrl)
        }
    }

    private func imagesGalleryView(images: [String]) -> some View {
    ...
    private func audioPlayerRow(audUrl: URL) -> some View {
    ...
    private var voteSegment: some View {
        HStack(spacing: 6) {
            Button(action: {
